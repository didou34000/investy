    ...
    const fetchNews = async () => {
      try {
        ...
        if (!Array.isArray(incoming) || incoming.length === 0) {
          // Fallback mock pour tester l'UX
          setItems([
            { id:"m1", title:"Point macro hebdo: inflation et taux", sourceId:"mock", sourceName:"Mock", contentSnippet:"Résumé synthétique pour test.", tags:["macro"], publishedAt:new Date().toISOString(), image:"" } as any,
            { id:"m2", title:"ETF Monde: diversification cœur", sourceId:"mock", sourceName:"Mock", contentSnippet:"Présentation ETF core.", tags:["etf"], publishedAt:new Date().toISOString(), image:"" } as any,
            { id:"m3", title:"Obligations: duration 5–7 ans", sourceId:"mock", sourceName:"Mock", contentSnippet:"Sensibilité aux taux.", tags:["obligations"], publishedAt:new Date().toISOString(), image:"" } as any,
          ]);
        } else {
          setItems(incoming);
        }
        ...
        setItems([
          { id:"m1", title:"Flux indisponible — aperçu de démonstration", sourceId:"mock", sourceName:"Mock", contentSnippet:"Contenu factice pour test.", tags:["demo"], publishedAt:new Date().toISOString(), image:"" } as any,
        ]);
        setError(null);
      } finally {
        setLoading(false);
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto max-w-6xl px-4 py-8">
        <SectionHeader
          title="Actu marché"
          subtitle={`Flux agrégés • ${fetchedAt ? `MAJ ${formatRelativeFR(fetchedAt as any)}` : 'Chargement…'}`}
        />

        {/* Controls */}
        <div className="space-y-4 mb-6 sticky top-16 z-10 bg-slate-50/80 backdrop-blur supports-[backdrop-filter]:bg-slate-50/60 py-3">
          {/* Period selector */}
          ...
          <>
            <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 [column-fill:_balance] before:box-inherit after:box-inherit">
              {displayedItems.map(item => (
                <article
                  key={item.id}
                  className="mb-4 break-inside-avoid bg-white rounded-2xl shadow hover:shadow-lg transition-all cursor-pointer hover-lift"
